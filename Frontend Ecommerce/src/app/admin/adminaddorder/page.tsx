...
// the `details` object that OrderView returns on validation failures — this
// was previously being silently dropped, hiding the real cause of 400s.
...
const AdminAddOrder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch customers and riders.
        // NOTE: '/api/myapp/v1/user/?role=customer' and '?role=rider' do not
        // appear in the ecommerce app's urls.py you've shared — they must
        // live in a separate users app. If these 404 or don't support
        // ?role= filtering, the dropdowns below will just stay empty
        // (caught below) without blocking order creation.
        try {
          const customersRes = await AxiosInstance.get('/api/myapp/v1/user/?role=customer');
          if (customersRes.data?.data && Array.isArray(customersRes.data.data)) {
            setCustomers(customersRes.data.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation — mirrors what the Order model actually requires:
    // customer_name, customer_email, customer_phone, delivery_address,
    // payment_method are all non-nullable on the model. city is genuinely
    // optional (null=True, blank=True) and intentionally NOT required here.
    const errors: string[] = [];
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Customer Email — required by the Order model, so marked required here too */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                  ...
                  {/* City — genuinely optional on the model */}
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      City
                    </label>
