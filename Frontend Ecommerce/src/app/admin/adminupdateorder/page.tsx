...
// Extracts a human-readable message from a DRF error response, including
// the `details` object OrderView returns on validation failures.
...
const UpdateOrder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setDataLoading(true);
      try {
        // Fetch supporting dropdown data. Customer/rider endpoints are
        // wrapped individually so a 404 on either doesn't take down the
        // whole page (see note in AdminAddOrder about that endpoint).
        ...
        try {
          ...
          console.warn('Could not load customers dropdown:', err);
        }
        ...
        try {
          ...
          console.warn('Could not load riders dropdown:', err);
        }

        // FIX: OrderView has no `v1/order/<id>/` path route — every other
        // view in this app (Address, ReturnRequest, Order.patch itself)
        // reads the id from a query param instead.
        const orderRes = await AxiosInstance.get('/api/myapp/v1/order/', { params: { id } });

        // FIX: OrderView.get() delegates to the generic list endpoint,
        // which returns { count, data: [...] } — an ARRAY — even when
        // filtered down to one row via ?id=. Handle both shapes defensively.
        let orderData: any = null;
        const responseData = orderRes.data?.data;
        if (Array.isArray(responseData)) {
          orderData = responseData.find((o: any) => String(o.id) === String(id)) || responseData[0] || null;
        } else if (responseData && typeof responseData === 'object') {
          orderData = responseData;
        } else if (orderRes.data && typeof orderRes.data === 'object' && !Array.isArray(orderRes.data)) {
          orderData = orderRes.data;
        }
        ...
        // OrderSerializer exposes the nested items as `order_details`
        // (see serializers.py), with `items` as a plain fallback in case
        // your backend response shape differs.
        const rawItems = orderData.order_details || orderData.items;

        if (Array.isArray(rawItems)) {
          const items = rawItems.map((item: any) => {
            const isSalesProduct = item.sales_product !== null && item.sales_product !== undefined;
            const productType: 'product' | 'sales_product' = isSalesProduct ? 'sales_product' : 'product';
            const productId = isSalesProduct ? item.sales_product : item.product;
            ...
            if (unitPrice === 0 && productId) {
              if (productType === 'product') {
                ...
                const salesProduct = loadedSalesProducts.find((p) => p.id === productId);
                if (salesProduct) {
                  unitPrice = salesProduct.final_price;
                  totalPrice = salesProduct.final_price * quantity;
                }
        ...
        console.error('Error fetching data:', error);
        toast.error(extractErrorMessage(error, 'Failed to load order data'), {
          position: "top-center",
          autoClose: 4000,
          theme: "dark",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.customer_email.trim()) errors.push('Customer email is required');
    if (!formData.customer_phone.trim()) errors.push('Customer phone is required');
    if (!formData.delivery_address.trim()) errors.push('Delivery address is required');

    const validItems = orderItems.filter(item => item.product_id !== null);
    if (validItems.length === 0) errors.push('Please add at least one product');
    ...
    try {
      const payload = {
        ...
        // NOTE: OrderView._update_order_items() doesn't currently read
        // delete_items — it replaces all items on every update regardless.
        // Sending this is harmless (ignored) but not doing anything either.
        delete_items: itemsToDelete
      };

      // FIX: OrderView has no `put` method — only post/get/patch/delete —
      // so PUT was returning 405 on every update attempt. Also switched to
      // the ?id= query param convention used everywhere else in this app.
      const response = await AxiosInstance.patch('/api/myapp/v1/order/', payload, {
        params: { id },
      });
      ...
      console.error('Error updating order:', error);
      toast.error(extractErrorMessage(error, 'Failed to update order'), {
        position: "top-center",
        autoClose: 4000,
        theme: "dark",
      });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center p-6">
        <div className="text-center p-10 max-w-md backdrop-blur-xl bg-white/5 rounded-3xl border border-white/10 shadow-2xl">
          ...
          <button
            onClick={() => router.push('/admin/adminorder')}
            className="group relative px-8 py-3 bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl font-semibold text-white shadow-lg shadow-amber-500/50 hover:shadow-amber-500/70 transition-all duration-300 hover:scale-105"
          >
            <span className="relative z-10">Return to Orders</span>
          </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-400/80 mt-1">
                      Requires a backend fix to _update_order_items — see notes.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value="in_process" className="bg-gray-900">In Process</option>
                    ...
                    {itemsToDelete.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-white/10">
                        <p className="text-sm text-red-400 flex items-center gap-2">
                          ...
                          {itemsToDelete.length} item(s) will be removed (existing items are fully replaced on save regardless)
                        </p>
                      </div>
                    )}
                  </div>
