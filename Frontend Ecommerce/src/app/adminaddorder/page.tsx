...
//   has_discount: boolean;
//   image: string | null;
//   salesprod_has_category: number | null;
//   category_data: any | null;
//   images: Array<{
//     id: number;
//     image_url: string;
//     alt_text: string;
...
//         const [productsRes, salesProductsRes, customersRes, ridersRes] = await Promise.all([
//           AxiosInstance.get('/api/myapp/v1/dropdown/product/'),
//           AxiosInstance.get('/api/myapp/v1/dropdown/sales/product/'), // Fixed: Added leading slash
//           AxiosInstance.get('/api/myapp/v1/user/?role=customer'),
//           AxiosInstance.get('/api/myapp/v1/user/?role=rider')
//         ]);
...
//             const salesProduct = salesProducts.find(p => p.id === item.product_id);
//             if (salesProduct) {
//               total += parseFloat(salesProduct.final_price) * item.quantity;
//             }
//           }
//         }
//       });
...
//                                   : salesProducts.map(product => (
//                                       <option key={product.id} value={product.id} className="bg-gray-900">
//                                         {product.name} - PKR {parseFloat(product.final_price).toFixed(2)} 
//                                         {product.has_discount && ` (${product.discount_percent}% off)`}
//                                       </option>
//                                     ))}
//                               </select>
//                               <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
