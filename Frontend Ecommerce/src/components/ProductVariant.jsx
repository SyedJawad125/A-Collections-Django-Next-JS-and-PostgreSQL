...
const AdminProductVariant = () => {
  ...
  const { permissions = {
    create_productvariant: false,
    read_productvariant: false,
    update_productvariant: false,
    delete_productvariant: false,
  ...
  useEffect(() => {
    const fetchVariants = async () => {
      if (!permissions.read_productvariant) { setIsLoading(false); return; }
      setIsLoading(true);
      try {
        ...
        toast.error(error.response?.data?.message || 'Failed to load product variants');
        setRecords([]);
      } finally {
        setIsLoading(false);
      }
  ...
  }, [refreshKey, pagination.currentPage, pagination.limit, permissions.read_productvariant]);
    ...
    const fetchOptions = async () => {
      ...
      try {
        ...
        if (productsRes.status === 'fulfilled') {
          const arr = productsRes.value?.data?.data;
  ...
  // ---------- Modal helpers ----------
  const resetForm = () => {
    setForm({ product: '', size: '', colors: [], material: '', stock_quantity: '', additional_price: '0', is_active: true });
    setEditingRecord(null);
  };

  const handleAdd = () => {
    if (!permissions.create_productvariant) { toast.error('You do not have permission to add variants'); return; }
    resetForm();
    setModalOpen(true);
  };

  const handleEdit = (record) => {
    if (!permissions.update_productvariant) { toast.error('You do not have permission to update variants'); return; }
  ...
  const handleDelete = async (id) => {
    if (!permissions.delete_productvariant) { toast.error('You do not have permission to delete variants'); return; }
    if (!window.confirm('Are you sure you want to delete this variant?')) return;
    try {
      await AxiosInstance.delete('/api/myapp/v1/product/variant/', { params: { id } });
      toast.success('Variant deleted successfully');
      setRefreshKey(k => k + 1);
    } catch (error) {
      console.error('Error deleting variant:', error);
      toast.error(error.response?.data?.error || 'Error deleting variant');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRecord) {
        await AxiosInstance.patch('/api/myapp/v1/product/variant/', { id: editingRecord.id, ...payload }, {
          params: { id: editingRecord.id },
        });
        toast.success('Variant updated successfully');
      } else {
        await AxiosInstance.post('/api/myapp/v1/product/variant/', payload);
      ...
      const msg = data?.error || data?.detail ||
        (typeof data === 'object' ? JSON.stringify(data) : 'Error saving variant');
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  if (!permissions.read_productvariant) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center">
        <div className="text-center p-8 max-w-md">
          <h2 className="text-2xl text-amber-400 mb-4">Access Denied</h2>
          <p className="text-gray-300 mb-6">You don't have permission to view product variants.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 py-12 px-4">
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-slate-700/50 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto my-8">
            <div className="sticky top-0 bg-slate-900/95 backdrop-blur-xl border-b-2 border-slate-700/50 p-6 rounded-t-3xl flex items-center justify-between">
              <h2 className="text-2xl font-bold bg-gradient-to-r from-amber-400 via-orange-400 to-red-400 bg-clip-text text-transparent">
                {editingRecord ? 'Edit Variant' : 'Add New Variant'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="space-y-2">
                <label className="text-amber-300 font-semibold text-sm uppercase tracking-wider">Product *</label>
                <select name="product" value={form.product} onChange={handleFormChange} disabled={loadingOptions}
                  ...
                  {productOptions.map(p => (
                    <option key={p.id} value={p.id} className="bg-slate-900">{p.name} — Rs {p.price}</option>
                  ))}
                </select>
              </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          <div className="text-center py-20 text-amber-400">Loading variants...</div>
                ...
                {filteredRecords.map(item => (
                  <tr key={item.id} className="border-t border-gray-700 hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 text-right space-x-2">
                      {permissions.update_productvariant && (
                        <button onClick={() => handleEdit(item)} className="p-2 bg-amber-600/90 rounded-lg hover:bg-amber-600 transition-colors" aria-label="Edit variant">✏️</button>
                      )}
                      {permissions.delete_productvariant && (
                        <button onClick={() => handleDelete(item.id)} className="p-2 bg-red-600/90 rounded-lg hover:bg-red-600 transition-colors" aria-label="Delete variant">🗑️</button>
                      )}
                    </td>
                  </tr>
                ))}
