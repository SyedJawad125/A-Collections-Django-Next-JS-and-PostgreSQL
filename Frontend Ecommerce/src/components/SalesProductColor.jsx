...
const SalesProductColor = () => {
  const router = useRouter();
  const { permissions = {
    create_sales_productcolor: false,
    read_sales_productcolor: false,
    update_sales_productcolor: false,
    delete_sales_productcolor: false,
  ...
  useEffect(() => {
    const fetchColors = async () => {
      if (!permissions.read_sales_productcolor) { setIsLoading(false); return; }
      setIsLoading(true);
      try {
        const res = await AxiosInstance.get('/api/myapp/v1/sales/product/color/', {
        ...
        console.error('Error fetching sales colors:', error);
        toast.error(error.response?.data?.message || 'Failed to load sales colors');
        setRecords([]);
      } finally {
        setIsLoading(false);
      }
  ...
  }, [refreshKey, pagination.currentPage, pagination.limit, permissions.read_sales_productcolor]);
  ...
  const handleAdd = () => {
    if (!permissions.create_sales_productcolor) { toast.error('You do not have permission to add sales colors'); return; }
    resetForm();
    setModalOpen(true);
  };

  const handleEdit = (record) => {
    if (!permissions.update_sales_productcolor) { toast.error('You do not have permission to update sales colors'); return; }
  ...
  const handleDelete = async (id) => {
    if (!permissions.delete_sales_productcolor) { toast.error('You do not have permission to delete sales colors'); return; }
    if (!window.confirm('Are you sure you want to delete this sales color?')) return;
    try {
      await AxiosInstance.delete('/api/myapp/v1/sales/product/color/', { params: { id } });
      toast.success('Sales color deleted successfully');
      setRefreshKey(k => k + 1);
    } catch (error) {
      console.error('Error deleting sales color:', error);
      toast.error(error.response?.data?.error || 'Error deleting sales color');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRecord) {
        await AxiosInstance.patch('/api/myapp/v1/sales/product/color/', { id: editingRecord.id, ...payload }, {
          params: { id: editingRecord.id },
        });
        toast.success('Sales color updated successfully');
      } else {
        await AxiosInstance.post('/api/myapp/v1/sales/product/color/', payload);
        toast.success('Sales color added successfully');
      }
      setModalOpen(false);
      resetForm();
      setRefreshKey(k => k + 1);
    } catch (error) {
      console.error('Error saving sales color:', error);
      const data = error.response?.data;
      const msg = data?.error || data?.detail ||
        (typeof data === 'object' ? JSON.stringify(data) : 'Error saving sales color');
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  if (!permissions.read_sales_productcolor) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center">
        <div className="text-center p-8 max-w-md">
          <h2 className="text-2xl text-amber-400 mb-4">Access Denied</h2>
          <p className="text-gray-300 mb-6">You don't have permission to view sales colors.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black py-16 px-4 relative overflow-hidden">
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-slate-700/50 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto my-8">
            <div className="sticky top-0 bg-slate-900/95 backdrop-blur-xl border-b-2 border-slate-700/50 p-6 rounded-t-3xl flex items-center justify-between">
              <h2 className="text-2xl font-bold bg-gradient-to-r from-amber-400 via-orange-400 to-red-400 bg-clip-text text-transparent">
                {editingRecord ? 'Edit Sales Color' : 'Add New Sales Color'}
              </h2>
      ...
      <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    Manage sales product colors
                  </p>
                </div>

                <div className="flex gap-3 flex-wrap mt-4 md:mt-0">
                  {permissions.create_sales_productcolor && (
                    <button
                      onClick={handleAdd}
                  ...
                  >
                    ...
                    <span className="relative z-10">Back to Variants</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        {isLoading ? (
          <div className="text-center py-20 text-amber-400">Loading sales colors...</div>
                ...
                {filteredRecords.map(item => (
                  <tr key={item.id} className="border-t border-gray-700 hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 text-right space-x-2">
                      {permissions.update_sales_productcolor && (
                        <button onClick={() => handleEdit(item)} className="p-2 bg-amber-600/90 rounded-lg hover:bg-amber-600 transition-colors" aria-label="Edit sales color">✏️</button>
                      )}
                      {permissions.delete_sales_productcolor && (
                        <button onClick={() => handleDelete(item.id)} className="p-2 bg-red-600/90 rounded-lg hover:bg-red-600 transition-colors" aria-label="Delete sales color">🗑️</button>
                      )}
                    </td>
                  </tr>
                ))}
...
export default SalesProductColor;
