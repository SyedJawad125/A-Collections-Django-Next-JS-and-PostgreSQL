...
//   if (!hasReadPermission) {
//     return (
//       <div className="w-full h-full bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center p-6">
    ...
    return (
      <div className="w-full h-full bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center p-6">
        <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-12 text-center max-w-md">
          ...
          <button
            onClick={() => router.push('/')}
            className="px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white rounded-xl font-semibold shadow-lg shadow-amber-500/25 transition-all duration-200 hover:scale-105"
          >
            Return to Dashboard
          </button>
        </div>
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b from-gray-900 to-gray-800 p-6 overflow-auto">
      ...
      <div className="mb-8">
        ...
        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-5 hover:border-amber-500/40 transition-all">
            <div className="flex items-center justify-between mb-2">
              <span className="text-slate-400 text-sm font-medium">Total Images</span>
          ...
          <div className="bg-gradient-to-br from-amber-900/20 to-amber-950/30 backdrop-blur-sm border border-amber-700/30 rounded-xl p-5 hover:border-amber-600/40 transition-all">
            ...
            <p className="text-3xl font-bold text-amber-400">{currentPage} / {totalPages || 1}</p>
          </div>

          <div className="bg-gradient-to-br from-yellow-900/20 to-yellow-950/30 backdrop-blur-sm border border-yellow-700/30 rounded-xl p-5 hover:border-yellow-600/40 transition-all">
            <div className="flex items-center justify-between mb-2">
              <span className="text-slate-400 text-sm font-medium">Showing</span>
              <Eye className="w-5 h-5 text-yellow-400" />
            </div>
            <p className="text-3xl font-bold text-yellow-400">{currentImages.length}</p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-4 items-center">
            <div className="flex-1 min-w-[250px] relative">
              ...
              <input
                ...
                value={searchTerm}
                onChange={handleSearch}
                className="w-full pl-12 pr-4 py-3 bg-slate-900/50 text-white border border-slate-700/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all placeholder:text-slate-500"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={itemsPerPage}
                onChange={handleLimitChange}
                className="px-4 py-3 bg-slate-900/50 text-white border border-slate-700/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all cursor-pointer"
              >
                <option value="12">12 per page</option>
                <option value="24">24 per page</option>
                <option value="36">36 per page</option>
      ...
      {/* Images Grid - LUXURY DESIGN (UNCHANGED) */}
      {!isLoading && (
        <>
          {currentImages.length > 0 ? (
            <>
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg font-medium transition-all ${
                            currentPage === pageNum
                              ? 'bg-gradient-to-r from-amber-600 to-yellow-500 text-slate-900 shadow-lg shadow-amber-500/25'
                              : 'bg-slate-800/50 text-slate-300 hover:bg-slate-700/50 border border-slate-700/50 hover:border-slate-600/50'
                          }`}
                        >
                          {pageNum}
            ...
            <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-12 text-center">
              ...
              {hasCreatePermission && !searchTerm && (
                <button
                  className="group relative inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-600 to-yellow-500 text-slate-900 font-semibold rounded-full shadow-2xl shadow-amber-500/50 hover:shadow-amber-500/70 transform hover:scale-105 transition-all duration-300"
                  onClick={handleAddImage}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-400 to-yellow-400 opacity-0 group-hover:opacity-100 rounded-full transition-opacity duration-300"></div>
                  <div className="relative flex items-center space-x-2">
      ...
      {imageModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-slate-700/50 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]">
            ...
            <form onSubmit={saveImage} className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-medium text-amber-300 uppercase tracking-wider mb-2">
                  Image Name <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <select
                  value={imageForm.category}
                  onChange={(e) => setImageForm({ ...imageForm, category: e.target.value })}
                  className="w-full px-4 py-3 bg-slate-900/50 text-white border-2 border-slate-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all cursor-pointer"
                >
                  <option value="">No category</option>
                  {categories.map((cat) => (
                    <option key={cat.id} value={cat.id}>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <label className="cursor-pointer px-6 py-3 bg-slate-800/50 hover:bg-slate-700/50 text-slate-300 hover:text-white border-2 border-slate-700/50 hover:border-amber-500/50 rounded-xl transition-all">
                    {imageForm.image ? 'Change Image' : editingImage ? 'Change Image' : 'Upload Image'}
                    <input
                      type="file"
                      accept="image/*"
            ...
            <div className="p-6 pt-0">
              ...
              <div className="space-y-3">
                {categories.map((cat) => (
                  <div
                    key={cat.id}
                    className="flex items-center justify-between p-4 bg-slate-900/50 border-2 border-slate-700/50 rounded-xl hover:border-amber-500/40 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      {cat.image && (
                        <img
