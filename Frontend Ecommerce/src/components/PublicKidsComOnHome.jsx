...
//             fetchKidsProducts(newPage, pagination.limit);
...
//         fetchKidsProducts(1, newLimit); // Reset to page 1 when changing limit
//     };
...
//                                 />
//                             </svg>
//                             <h3 className="mt-4 text-lg font-medium text-black">No kids products found</h3>
...
const KidsOnHome = () => {
    ...
    const fetchKidsProducts = async (page = 1, limit = 16) => {
        ...
        try {
            ...
            // Response structure: { message, count, data }
            const responseData = res?.data;
            ...
            console.log('Products data array:', dataArr); // Debug
            console.log('First product:', dataArr[0]); // Debug
            
            // Process the products to include proper image URLs
            const processedProducts = dataArr.map(product => {
                console.log('Processing product:', product.name); // Debug
                console.log('Product image_urls:', product.image_urls); // Debug
                console.log('Product images array:', product.images); // Debug
                
                // Try multiple image sources in order
                let mainImage = '/default-product-image.jpg';
                
                // 1. First try image_urls array (strings)
                if (Array.isArray(product.image_urls) && product.image_urls.length > 0) {
                    const firstUrl = product.image_urls[0];
                    console.log('First URL from image_urls:', firstUrl); // Debug
                    
                    if (firstUrl && typeof firstUrl === 'string') {
                        if (firstUrl.startsWith('http')) {
                            mainImage = firstUrl;
                        } else if (firstUrl.startsWith('/')) {
                            mainImage = `${baseURL}${firstUrl}`;
                        } else {
                            mainImage = `${baseURL}/${firstUrl}`;
                        }
                    }
                }
                // 2. Then try images array (objects with image_url property)
                else if (Array.isArray(product.images) && product.images.length > 0) {
                    const firstImageObj = product.images[0];
                    if (firstImageObj?.image_url) {
                        const imgUrl = firstImageObj.image_url;
                        console.log('First URL from images array:', imgUrl); // Debug
                        
                        if (imgUrl.startsWith('http')) {
                            mainImage = imgUrl;
                        } else if (imgUrl.startsWith('/')) {
                            mainImage = `${baseURL}${imgUrl}`;
                        } else {
                            mainImage = `${baseURL}/${imgUrl}`;
                        }
                    }
                }
                // 3. Try direct image property if available
                else if (product.image && typeof product.image === 'string') {
                    const imgUrl = product.image;
                    console.log('URL from image property:', imgUrl); // Debug
                ...
                console.log('Final mainImage URL:', mainImage); // Debug
                            ...
                            >
                                ...
                                <div className="relative w-full h-48 overflow-hidden bg-gray-50">
                                    <img
                                        ...
                                        className="w-full h-full object-cover transform group-hover:scale-110 transition duration-500"
                                        onError={(e) => {
                                            console.error(`Failed to load image: ${item.mainImage}`); // Debug
                                            e.target.src = '/default-product-image.jpg';
