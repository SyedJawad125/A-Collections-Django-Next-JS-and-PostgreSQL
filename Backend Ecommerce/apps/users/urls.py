...
from .views import (
    ...
    ForgetPasswordView, VerifyOTPView, ResetPasswordView,
    ChangePasswordView,
    VerifyLinkView,
    AccountActivateView,
    EmployeeView, EmployeeToggleView,
...
urlpatterns = [
    ...
    path('v1/forget/password/', ForgetPasswordView.as_view(), name='forget-password'),   # Step 1
    path('v1/verify/otp/',      VerifyOTPView.as_view(),      name='verify-otp'),        # Step 2
    path('v1/reset/password/',  ResetPasswordView.as_view(),  name='reset-password'),    # Step 3

    # ── Account Activation ─────────────────────────────────────────────────────
    # OTP-based activation (new)
    path('v1/account/activate/', AccountActivateView.as_view(), name='account-activate'),

    # Legacy token link verification (backward compat only)
    path('v1/verify/link/', VerifyLinkView.as_view(), name='verify-link'),
