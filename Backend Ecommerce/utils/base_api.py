...
class BaseView(APIView):
    ...
    def post_(self, request):
        try:
            ...
            logger.exception("BaseView.post_ failed")
            return Response(create_response(str(e)), status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if request.query_params.get('id'):
                ...
                instances = self.serializer_class.Meta.model.objects.filter(deleted=False,
                                                                            **self.extra_filters).order_by(order_by)
                if self.filterset_class:
                    # FIXED: previously `self.filterset_class(request.GET,
                    # queryset=instances)` — without `request=request`,
                    # any FilterSet whose `.qs` property or filter
                    # methods read `self.request` (e.g. to distinguish
                    # staff from anonymous callers) always sees
                    # `self.request is None`, which silently produces the
                    # *most restrictive* behaviour for every caller,
                    # staff included. Passing request here is required
                    # for that pattern to work at all.
            ...
            logger.exception("BaseView.get_ failed")
            return Response(create_response(str(e)), status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def patch_(self, request):
        try:
            ...
            logger.exception("BaseView.patch_ failed")
            return Response(create_response(str(e)), status=status.HTTP_400_BAD_REQUEST)

    def delete_(self, request):
        """
        NOTE: this generic soft-delete assumes the model looks like
        TimeUserStamps (a plain `deleted` boolean + an `updated_by` FK).
        It works correctly for Category/Tag/BlogPost. It does NOT work
        correctly for models with a richer soft-delete shape — e.g.
        Comment, which also tracks `deleted_at`/`deleted_by` and has its
        own `soft_delete()` method with different validation rules.
        Setting `instance.updated_by` on a model with no such field is
        silently a no-op rather than an error, so this fails quietly
        instead of loudly. Views for those models should override
        delete_/delete() and call the model's own soft-delete method
        instead of relying on this default. See CommentView.delete() for
        an example.
        """
        try:
            if request.query_params.get('id'):
            ...
            logger.exception("BaseView.delete_ failed")
            return Response(create_response(str(e)), status=status.HTTP_500_INTERNAL_SERVER_ERROR)